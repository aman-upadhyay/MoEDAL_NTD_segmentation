...
binary_dir = "Data_aman/b2_250/"
binary1_dir = "Data_aman/b1_250/"
dirty_dir = "Data_aman/d1_250/"

binary_dir_filename = os.listdir(binary_dir)
binary1_dir_filename = os.listdir(binary1_dir)
...
for filename in tqdm(binary1_dir_filename):
	i += 1
	d = filename.replace('b', 'd')
	if i < len(binary1_dir_filename) * 0.8:
		image_b = Image.open(os.path.join(binary1_dir, filename))
		image_d = Image.open(os.path.join(dirty_dir, d))
		binary_array_80.append(np.asarray(image_b)[:, :, 0].reshape(300, 360, 1))
		dirty_array_80.append(np.asarray(image_d).reshape(300, 360, 1))
	else:
